...
export default async function ArticleListPage({
  ...
  const visibleArticles = articles.filter((a) => canViewContent(plan, a.allowedPlans));

  return (
    <div className="space-y-5">
      <h1 className="text-xl font-bold text-brand-green-dark">ブログ記事一覧</h1>
      ...
      <form className="flex flex-col gap-2 sm:flex-row" method="get">
        <input
          ...
          className="flex-1 rounded-lg border border-brand-beige px-3 py-2 text-sm"
        />
        <select name="category" defaultValue={searchParams.category ?? ""} className="rounded-lg border border-brand-beige px-3 py-2 text-sm">
        ...
        <button type="submit" className="rounded-lg bg-brand-green px-4 py-2 text-sm font-semibold text-white hover:bg-brand-green-dark">
          検索
        </button>
      </form>
